...
export const MemberVerification: React.FC<MemberVerificationProps> = ({
  ...
  const [selectedStatus, setSelectedStatus] = useState<'all' | 'verified' | 'unverified' | 'visited' | 'not-visited'>('all');
  ...
  const filteredFolderMembers = useMemo(() => {
    return folderMembers.filter(item => {
      ...
      if (selectedStatus === 'verified') {
        ...
        matchesStatus = !!item.existingAccVisited;
      } else if (selectedStatus === 'not-visited') {
        matchesStatus = !item.existingAccVisited;
      }
              ...
              <div className="p-5 border-b border-slate-100 space-y-4">
                ...
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {([
                    ...
                    { id: 'visited', label: 'Visited', count: activeFolderStats.visited }
                  ] as const).map((tab) => {
                          ...
                          >
                            ...
                            <td className="py-3.5 px-5">
                              <div className="flex flex-col gap-1">
                                ...
                                {item.existingAccVisited && (
                                  <span className="text-[9px] text-teal-600 font-semibold flex items-center gap-1 mt-0.5">
                        ...
                        <div className="space-y-6">
                          <div>
                            ...
                            <div className="bg-white border border-slate-100 rounded-xl p-5 shadow-sm space-y-4">
                              ...
                              <div className="flex items-center justify-between p-3 bg-slate-50/50 hover:bg-slate-50 rounded-xl border border-slate-200/50 transition-colors">
                                <div className="flex items-center gap-2.5">
                                  <MapPin className={`w-5 h-5 ${formVisited ? 'text-teal-600' : 'text-slate-400'}`} />
                                  <div>
                                    <span className="text-xs font-bold text-slate-700 block">Physical Visit Logged</span>
                                    <span className="text-[10px] text-slate-400 block">Residency check performed</span>
                                  </div>
                                </div>
                                <button
                                  type="button"
                                  onClick={() => setFormVisited(!formVisited)}
                                  className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                                    formVisited ? 'bg-teal-600' : 'bg-slate-200'
                                  }`}
                                >
                                  <span
                                    className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow-xs ring-0 transition duration-200 ease-in-out ${
                                      formVisited ? 'translate-x-4' : 'translate-x-0'
                                    }`}
                                  />
                                </button>
                              </div>
