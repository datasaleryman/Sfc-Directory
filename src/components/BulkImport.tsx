  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-6">
      ...
      <div className="flex items-center justify-between border-b border-slate-100 pb-4">
        ...
        <button
          onClick={onCancel}
          className="text-xs font-semibold text-slate-500 hover:text-slate-800 border border-slate-200 rounded-xl px-4 py-2 hover:bg-slate-50 transition-all cursor-pointer"
        >
          Back to Dashboard
        </button>
      </div>
      ...
      {!previewData && !importSummary && (
        <div className="space-y-4">
          ...
          <button
            ...
            disabled={loadingPreview}
            className="w-full sm:w-auto px-6 py-3 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 text-white font-semibold text-sm rounded-xl shadow-lg shadow-indigo-50 active:scale-[0.99] transition-all flex items-center justify-center gap-2 cursor-pointer"
          >
            {loadingPreview ? (
              <>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 text-center">
              ...
              <span className="text-sm font-bold text-slate-700 mt-1 block">{previewData.detectedSeparator}</span>
            </div>
            <div className="bg-blue-50/50 p-3 rounded-xl border border-blue-100 text-center">
              ...
              <span className="text-base font-bold text-blue-700 mt-1 block">{previewData.summary.total}</span>
            </div>
            <div className="bg-emerald-50/50 p-3 rounded-xl border border-emerald-100 text-center">
              ...
              <span className="text-base font-bold text-emerald-700 mt-1 block">{previewData.summary.valid}</span>
            </div>
            <div className="bg-amber-50/50 p-3 rounded-xl border border-amber-100 text-center">
              ...
              <span className="text-base font-bold text-amber-700 mt-1 block">{previewData.summary.duplicate}</span>
            </div>
            <div className="bg-rose-50/50 p-3 rounded-xl border border-rose-100 text-center col-span-2 md:col-span-1">
              <span className="block text-[10px] font-bold text-rose-500 uppercase">✖ Invalids</span>
              <span className="text-base font-bold text-rose-700 mt-1 block">{previewData.summary.invalid}</span>
            </div>
          </div>
          ...
          <div className="bg-slate-50 border border-slate-150 p-5 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="space-y-1">
              <h5 className="font-bold text-slate-700 text-sm">Choose Import Conflict Behavior</h5>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                Replace Barangay/Purok of Duplicates
              </button>
              <button
                type="button"
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 pt-4 border-t border-slate-100">
            <button
              ...
              className="w-full sm:w-auto px-6 py-3 bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-400 text-white font-semibold text-sm rounded-xl shadow-lg shadow-emerald-50 active:scale-[0.99] transition-all flex items-center justify-center gap-2 cursor-pointer"
            >
              {savingRecords ? (
                <>
                  <Loader2 className="w-4 h-4 animate-spin" />
      ...
      {importSummary && (
        <div className="bg-slate-50 border border-emerald-200 p-8 rounded-2xl flex flex-col items-center justify-center text-center space-y-6">
          <div className="w-16 h-16 bg-emerald-50 border border-emerald-200 rounded-full flex items-center justify-center text-emerald-600 shadow-md">
            <CheckCircle2 className="w-8 h-8" />
          </div>

          <div className="space-y-1.5 max-w-md">
            <h4 className="font-bold text-slate-800 text-xl font-display">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-white border border-slate-200 rounded-2xl p-4 w-full max-w-xl shadow-sm text-center">
            <div>
              <span className="block text-[10px] font-bold text-slate-400 uppercase">Delivered Rows</span>
              <span className="text-lg font-bold text-slate-700 block mt-1">{importSummary.total}</span>
            </div>
            <div>
              <span className="block text-[10px] font-bold text-emerald-500 uppercase">Saved to DB</span>
              <span className="text-lg font-bold text-emerald-700 block mt-1">{importSummary.saved}</span>
            </div>
            <div>
              <span className="block text-[10px] font-bold text-blue-500 uppercase">Replaced Dups</span>
              <span className="text-lg font-bold text-blue-700 block mt-1">{importSummary.replaced}</span>
            </div>
            <div>
              <span className="block text-[10px] font-bold text-slate-400 uppercase">Skipped Rows</span>
              <span className="text-lg font-bold text-slate-500 block mt-1">{importSummary.skipped}</span>
            </div>
          </div>

          <button
            onClick={handleReset}
            className="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-sm rounded-xl shadow-lg shadow-indigo-50 active:scale-[0.99] transition-all flex items-center justify-center gap-1.5 cursor-pointer"
          >
