              ...
              <div className="p-6 overflow-y-auto space-y-6 flex-1 text-slate-700 text-xs">
                ...
                <div className="space-y-3">
                  <h4 className="font-bold text-slate-900 text-sm flex items-center gap-2 font-display">
                    ...
                    <span>{selectedContact.isExistingAccount ? 'Attached Existing Account Files' : 'Uploaded PCU File Document'}</span>
                  </h4>

                  {selectedContact.isExistingAccount && selectedContact.uploadedFiles && selectedContact.uploadedFiles.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {selectedContact.uploadedFiles.map((file, idx) => (
                        <div 
                          key={idx} 
                          className="flex items-center justify-between p-3 bg-emerald-50 rounded-2xl border border-emerald-200/80 text-xs font-semibold group transition-all"
                        >
                          <div className="flex items-center gap-2 min-w-0 flex-1">
                            <FileText className="w-5 h-5 text-emerald-700 shrink-0" />
                            <div className="min-w-0 flex-1">
                              <p className="text-emerald-950 font-bold truncate" title={file.name}>
                                {file.name}
                              </p>
                              {file.uploadedAt && (
                                <p className="text-[9px] text-emerald-700/80 mt-0.5">
                          ...
                          <a 
                            ...
                            className="p-1.5 bg-white hover:bg-emerald-100 border border-emerald-200 text-emerald-700 rounded-lg shrink-0 transition-all flex items-center justify-center cursor-pointer ml-2"
                            title="Open File"
                          >
